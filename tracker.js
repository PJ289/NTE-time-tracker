...
function spawnHiddenSea(extraArgs) {
  if (!IS_SEA || !process.platform.startsWith('win')) return null;
...
function launchHtaGui(fileName, htmlContent) {
  ...
  // Use "cmd start" so mshta gets a visible window; windowsHide on mshta itself hides the HTA UI.
  const child = spawn('cmd.exe', ['/c', 'start', '""', 'mshta.exe', htaPath], {
    detached: true,
    stdio: 'ignore',
    windowsHide: true
  });
...
function restartTracker() {
  ...
  spawnHiddenSea(process.argv.slice(1));
  handleShutdown();
}
...
async function performAutoUpdate(updateInfo) {
  ...
  try {
    ...
    const workDir = path.dirname(currentExe);
    const psExe = currentExe.replace(/'/g, "''");
    ...
    const bat = [
      ...
      '  goto :eof',
      ')',
      'powershell -NoProfile -WindowStyle Hidden -Command "$env:NTE_SEA_BACKGROUND=\'1\'; Start-Process -LiteralPath \'' + psExe + '\' -WorkingDirectory \'' + psWork + '\' -WindowStyle Hidden"',
      'del "' + tmpExe + '"',
      'del "%~f0"'
    ].join('\r\n');
